...
// ---------- 任兩地支的關係查詢(與 bazi.js 的關係總表同一套定義) ----------
// 放在這裡(不放 bazi.js)是為了讓「流年變動」等純解讀功能不用 import 排盤引擎,
// 保持前端的動態載入分包(bazi.js 會拖進 lunar-javascript)。
const REL_TABLES = [
  [[['子', '丑'], ['寅', '亥'], ['卯', '戌'], ['辰', '酉'], ['巳', '申'], ['午', '未']], '六合'],
  [[['子', '未'], ['丑', '午'], ['寅', '巳'], ['卯', '辰'], ['申', '亥'], ['酉', '戌']], '害'],
  [[['子', '午'], ['丑', '未'], ['寅', '申'], ['卯', '酉'], ['辰', '戌'], ['巳', '亥']], '沖'],
  [[['寅', '巳'], ['巳', '申'], ['申', '寅'], ['丑', '戌'], ['戌', '未'], ['未', '丑'], ['子', '卯']], '刑'],
  [[['子', '酉'], ['卯', '午'], ['辰', '丑'], ['戌', '未'], ['寅', '亥'], ['巳', '申']], '相破'],
  [[['寅', '丑'], ['午', '亥'], ['卯', '申'], ['巳', '酉']], '暗合'],
  [[['申', '子'], ['子', '辰'], ['巳', '酉'], ['酉', '丑'], ['寅', '午'], ['午', '戌'], ['亥', '卯'], ['卯', '未']], '半合'],
  [[['申', '辰'], ['巳', '丑'], ['寅', '戌'], ['亥', '未']], '拱'],
  [[['寅', '卯'], ['卯', '辰'], ['寅', '辰'], ['巳', '午'], ['午', '未'], ['巳', '未'],
    ['申', '酉'], ['酉', '戌'], ['申', '戌'], ['亥', '子'], ['子', '丑'], ['亥', '丑']], '半會'],
];

/** 查任兩個地支之間成立的所有關係(回傳引擎代號:六合/害/沖/刑/相破/暗合/半合/拱/半會) */
export function relationsBetween(a, b) {
  if (!a || !b || a === b) return [];
  return REL_TABLES
    .filter(([table]) => table.some(([x, y]) => (x === a && y === b) || (y === a && x === b)))
    .map(([, rel]) => rel);
}
